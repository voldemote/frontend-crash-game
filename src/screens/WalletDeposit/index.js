...
const WalletDeposit = ({ showError }) => {
  ...
  const renderPaymentLine = (
    ...
    value,
    showSign = false,
    highlightValueGreen = false
  ) => {
    let formattedValue = _.round(value, 2).toFixed(2);
    formattedValue =
      getPaymentProviderTitle(paymentProvider) + ' ' + formattedValue;
    ...
    return (
      <div className={styles.paymentLine}>
        <span className={styles.paymentLineDescription}>{description}</span>
        <span
  ...
  const renderPaymentTotalCalculation = () => {
    return <>{renderPaymentLine('Total', depositValue, true, true)}</>;
  };

  const renderPaymentComposition = () => {
    return <>{renderPaymentLine('Transaction fees', getFees())}</>;
  };
