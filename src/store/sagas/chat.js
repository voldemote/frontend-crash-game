...
const rehydrate = function* () {
  ...
  const userIds = _.uniq(
    _.flatten(
      _.map(messagesByEvent, messages =>
        _.map(messages, message => _.get(message, 'userId'))
      )
    )
  );
  ...
  yield all(
    _.map(missingUserIds, userId => put(UserActions.fetch({ userId })))
  );
};
...
const fetchInitial = function* (action) {
  ...
  if (eventId) {
    ...
    if (response) {
      const messages = response.data;
      yield put(
        ChatActions.fetchInitialSucceeded({
          eventId,
          messages,
        })
      );
      ...
      yield put(
        ChatActions.fetchInitialFailed({
          eventId,
          error,
        })
      );
