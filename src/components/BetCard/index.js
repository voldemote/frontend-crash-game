    ...
    <div className={styles.betCardContainer} onClick={onClick}>
      <div className={styles.betCard}>
        ...
        <div className={styles.betCardContentContainer}>
          <ProfileContainer className={styles.profileInfo} user={user} />
          <div className={styles.betCardMarketQuestion}>{marketQuestion}</div>
          <div>{hot && <HotBetBadge theme={HotBetBadgeTheme.opacity04} />}</div>
        </div>
      </div>
      {renderFooter()}
    </div>
...
export default connect(mapStateToProps, null)(BetCard);
