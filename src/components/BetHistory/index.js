...
const BetHistory = () => {
    ...
    return (
        <div className={styles.container}>
            <Tabs value={selectedTab} onChange={handleTabChange} classes={{ indicator: tabsClasses.indicator }}>
                <Tab label="Last crashes" disableRipple classes={{ root: tabClasses.root }} />
                <Tab label="In game bets" disableRipple classes={{ root: tabClasses.root }} />
                {breakpoint > 'sm' && <Tab label="Chat" disableRipple classes={{ root: tabClasses.root }} />}
            </Tabs>
                    ...
                    <div className={styles.bets}>
                        {inGameBets.map(bet => (
                            <div key={bet.username} className={styles.inGameBet}>
                                <div className={styles.user}><UserAvatarIcon />{bet.username}</div>
            ...
            {breakpoint > 'sm' && (
                <TabPanel value={selectedTab} index={2}>
                    <Chat className={styles.chatContainer} inputClassName={styles.chatInput} />
                </TabPanel>
            )}
        </div>
    );
