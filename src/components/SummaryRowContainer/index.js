...
const SummaryRowContainer = ({ className, summaryRows }) => {
  ...
  const renderSingleSummaryRow = (
    ...
    return (
      ...
      >
        {key && (
          <span
            className={classNames(styles.key, keyBold ? styles.bold : null)}
          >
            {key}
          </span>
        )}
        {valueChildren ? (
          ...
          <span
            className={classNames(
              ...
              SelectionHelper.get(valueColor, {
  ...
  return <>{_.map(summaryRows, renderSummaryRow)}</>;
};
