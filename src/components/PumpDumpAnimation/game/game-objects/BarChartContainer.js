...
export class BarChartContainer extends Container {
    ...
    mergeBars() {
        const scale = { x: 1 / this.scale.x, y: 1 / this.scale.y };
        this.barScale = scale;
        ...
        for (let index = this.generatedBars.length - 1; index >= 0; index -= MERGE_BAR_COUNT) {
            ...
            bigBarConfig.x /= barsToMerge.length;
            ...
            this.mergeBarsTween(barsToMerge, bigBarConfig);
        }
    }
    ...
    setupBarChart(timeElapsed, crashFactor) {
        if (timeElapsed - this.previousThreshold >= INIT_CREATE_THRESHOLD * this.createThresholdMult) {
            this.previousThreshold = timeElapsed;
            this.generateBar(timeElapsed, crashFactor);
        }
    }
    ...
    generateBar(timeElapsed, crashFactor) {
        ...
        let barHeightIndex = Math.floor(Math.random() * randMax);
        if (Math.random() < 0.1) {
